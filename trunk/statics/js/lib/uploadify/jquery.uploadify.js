...
SWFUpload.prototype.RequeueUpload = function(a) {
	this.callFlash("RequeueUpload", [a]);
};
...
(function($) {
	...
	var methods = {
		...
		init: function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					buttonText: '上传新文件', // The text to use for the browse button
		...
		RequeueUpload : function(index){
			this.each(function() {
				// Create a reference to the jQuery DOM object
				var $this = $(this),
					swfuploadify = $this.data('uploadify');
				swfuploadify.RequeueUpload(index);
			});
		},
	...
	var handlers = {
		...
		onUploadSuccess: function(file, data, response) {
			...
			if ($.inArray('onUploadSuccess', settings.overrideEvents) < 0) {
				...
				if(a.success){
					swfuploadify.complate = true;
					$('#' + file.id).find('.data').html(' - Complete');
				}else{
					swfuploadify.complate = false;
					$('#' + file.id).find('.data').html(' - ' + a.msg);
				}
			}
